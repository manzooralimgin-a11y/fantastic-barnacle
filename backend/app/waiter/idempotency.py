...
logger = logging.getLogger("app.waiter.idempotency")

_IDEMPOTENCY_PREFIX = "waiter:orders:idempotency"
...
class WaiterOrderIdempotencyService:
    @classmethod
    ...
    async def claim_or_replay(
        ...
        *,
        ...
        await api_metrics.record_business_event("waiter.order.idempotency.request")
        ...
        if claimed:
            await api_metrics.record_business_event("waiter.order.idempotency.miss")
            log_event(
                logger,
                logging.INFO,
                "waiter_order_idempotency_claimed",
                scope=scope,
                request_source=request_source,
                endpoint=endpoint,
            )
        ...
        raise HTTPException(
            status_code=409,
            detail="Order with this Idempotency-Key is already being processed",
        )
    ...
    async def complete(
        ...
        *,
        ...
        await api_metrics.record_business_event("waiter.order.idempotency.completed")
        log_event(
            logger,
            logging.INFO,
            "waiter_order_idempotency_recorded",
            request_source=request_source,
            endpoint=endpoint,
            status_code=status_code,
        )
    ...
    async def complete_or_log(
        ...
        *,
        ...
        try:
            ...
            await api_metrics.record_business_event("waiter.order.idempotency.finalize_failure")
            log_event(
                logger,
                logging.ERROR,
                "waiter_order_idempotency_finalize_failed",
                request_source=request_source,
                endpoint=endpoint,
                error=str(exc),
            )
    ...
    async def release(
        ...
        *,
        ...
        try:
            ...
            await api_metrics.record_business_event("waiter.order.idempotency.released")
            log_event(
                logger,
                logging.WARNING,
                "waiter_order_idempotency_released",
                request_source=request_source,
                endpoint=endpoint,
                error=error,
            )
        except Exception:
            logger.debug("Failed to release waiter order idempotency claim", exc_info=True)
        ...
        *,
        ...
        while True:
            ...
            if record.get("request_hash") != request_hash:
                await api_metrics.record_business_event("waiter.order.idempotency.conflict")
                log_event(
                    logger,
                    logging.WARNING,
                    "waiter_order_idempotency_conflict",
                    scope=scope,
                    request_source=request_source,
                    endpoint=endpoint,
                )
                raise HTTPException(
                    status_code=409,
                    detail="Idempotency-Key is already used for a different waiter order",
                )

            if record.get("status") == "completed":
                await api_metrics.record_business_event("waiter.order.idempotency.hit")
                response = record.get("response")
                if not isinstance(response, dict):
                    raise HTTPException(
                        status_code=503,
                        detail="Waiter order idempotency returned an invalid stored response",
                    )
            ...
            await api_metrics.record_business_event("waiter.order.idempotency.wait")
            if elapsed_ms >= settings.reservation_idempotency_max_wait_ms:
                log_event(
                    logger,
                    logging.WARNING,
                    "waiter_order_idempotency_inflight_timeout",
                    scope=scope,
                    request_source=request_source,
                    endpoint=endpoint,
                    wait_ms=elapsed_ms,
    ...
    def _parse_record(raw: str) -> dict[str, Any]:
        try:
            ...
            raise HTTPException(
                status_code=503,
                detail="Waiter order idempotency returned an invalid record",
            ) from exc
    ...
    async def _redis() -> Any:
        try:
            ...
            await api_metrics.record_business_event("waiter.order.idempotency.backend_failure")
            log_event(
                logger,
                logging.ERROR,
                "waiter_order_idempotency_backend_unavailable",
                error=str(exc),
            )
            raise HTTPException(
                status_code=503,
                detail="Waiter order idempotency service unavailable",
            ) from exc
