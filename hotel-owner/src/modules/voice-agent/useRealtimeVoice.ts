...
export function useRealtimeVoice() {
  ...
  const pendingDecodedChunksRef = useRef<Map<string, Float32Array[]>>(
    new Map()
  );
  ...
  function trimPlaybackQueue() {
    const maxQueuedSamples = Math.floor(
      PCM_SAMPLE_RATE * PLAYBACK_MAX_QUEUE_SECONDS
    );
  ...
  function flushDecodedChunksInOrder({ force }: { force: boolean }) {
    if (force) {
      const remainingKeys = Array.from(
        pendingDecodedChunksRef.current.keys()
      ).sort(
        compareSequenceKeys
      );
  ...
  function schedulePlaybackQueue() {
    ...
    while (playbackQueueRef.current.length > 0) {
      ...
      const shouldForceScheduling =
        queueSeconds >=
        PLAYBACK_MAX_QUEUE_SECONDS * PLAYBACK_FORCE_SCHEDULE_RATIO;
      ...
      const channelData = new Float32Array(nextChunk.length);
      channelData.set(nextChunk);
      ...
      void (async () => {
        try {
          ...
          for (
            ...
            const chunkBytes = alignedBytes.subarray(
              offset,
              offset + chunkByteSize
            );
